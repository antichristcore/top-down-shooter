...
class Player:
    ...
    def update(self, delta_time, move_vector, arena_bounds):
        self.velocity = move_vector.normalize() * self.speed if move_vector.length > 0 else Vector2(0, 0)
    ...
    def shoot(self, target_pos):
        if not self.can_shoot():
            return None
        direction = Vector2(target_pos[0] - self.position.x, target_pos[1] - self.position.y)
