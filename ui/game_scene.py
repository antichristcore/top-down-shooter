...
try:
    from arcade.camera import Camera2D as ArcadeCamera
except ImportError:  # pragma: no cover
    from arcade.camera import Camera as ArcadeCamera
...
class GameScene(arcade.View):
    def __init__(self, settings, score_system):
        super().__init__()
    ...
    def on_show_view(self):
        if not self.camera:
            ...
            self.gui_camera = self._create_camera()
    ...
    def _create_camera(self):
        try:
            return ArcadeCamera(self.window.width, self.window.height)
        except TypeError:
            return ArcadeCamera()
    ...
    def check_win_loss(self):
        if self.player.hp <= 0:
            ...
            self.window.game_over_view.configure(self.score, False)
        ...
        if self.wave_spawner and self.wave_spawner.wave_complete() and not self.enemies:
            if self.level_index + 1 < len(self.levels):
                ...
                self.score_system.submit(self.score)
                self.window.game_over_view.configure(self.score, True)
                self.window.show_view(self.window.game_over_view)
